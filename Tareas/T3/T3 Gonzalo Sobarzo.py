...
camera_pos = np.array([2.0, 0.75, 7.75]) #posicion inicial 
camera_radius = 2 #radio de la camara del sist de coord
theta = 3*np.pi/2 #angulo theta del sist de coord
# A class to store the application control
class Controller:
    def __init__(self):
        self.fillPolygon = True
        self.showAxis = True
        self.viewPos = np.array(camera_pos)
...
def createHouse(pipeline):
    muralla=createGPUShape(pipeline,bs.createTextureQuad(1.0,1.0)) #cuadrado con la textura de la muralla
    muralla.texture = es.textureSimpleSetup(
        getAssetPath("wall1.jpg"), GL_REPEAT, GL_REPEAT, GL_LINEAR_MIPMAP_LINEAR, GL_NEAREST)
    glGenerateMipmap(GL_TEXTURE_2D)

    techo=createGPUShape(pipeline,bs.createTextureQuad(1.0,1.0)) #cuadrado con la textura del techo 
    techo.texture = es.textureSimpleSetup(
        getAssetPath("roof4.jpg"), GL_REPEAT, GL_REPEAT, GL_LINEAR_MIPMAP_LINEAR, GL_NEAREST)
    glGenerateMipmap(GL_TEXTURE_2D)

    techoNode=sg.SceneGraphNode('techo') #parte 1 del techo
    techoNode.transform=tr.matmul([tr.translate(0,0.7,-0.35),tr.rotationX(np.pi/4)])
    techoNode.childs += [techo]

    techoNode2=sg.SceneGraphNode('techo') #parte 2 del techo 
    techoNode2.transform=tr.matmul([tr.translate(0,0.7,0.35),tr.rotationX(-np.pi/4)])
    techoNode2.childs += [techo]

    techos=sg.SceneGraphNode('techos') #nodo para las dos partes del techo 
    techos.transform=tr.translate(0,0,0.5)
    techos.childs += [techoNode]
    techos.childs += [techoNode2]

    murallaNode=sg.SceneGraphNode('murocasa') #muro 1 de la casa 
    murallaNode.transform=tr.matmul([tr.translate(0.5,0.0,0.5),tr.rotationY(np.pi/2)])
    murallaNode.childs+=[muralla]

    murallaNode2=sg.SceneGraphNode('murocasa') #muro 2 de la casa
    murallaNode2.childs+=[muralla]

    murallaNode3=sg.SceneGraphNode('murocasa') #muro 3 de la casa
    murallaNode3.transform=tr.translate(0.0,0.0,1.0)
    murallaNode3.childs+=[muralla]

    murallaNode4=sg.SceneGraphNode('murocasa') #muro 4 de la casa
    murallaNode4.transform=tr.matmul([tr.translate(-0.5,0.0,0.5),tr.rotationY(np.pi/2)])
    murallaNode4.childs+=[muralla]

    parteabajo=sg.SceneGraphNode('parteabajocasa') #parte baja de la casa
    parteabajo.childs +=[murallaNode]
    parteabajo.childs +=[murallaNode2]
    parteabajo.childs +=[murallaNode3]
    parteabajo.childs +=[murallaNode4]

    casa=sg.SceneGraphNode('casa') #se juntan las 2 partes de la casa
    casa.transform=tr.uniformScale(0.7)
    casa.childs += [techos]
    casa.childs += [parteabajo]

    scene=sg.SceneGraphNode('system') #escena final
    scene.childs+=[casa]
    return scene

def createWall(pipeline):
    Barreraa = createGPUShape(pipeline, bs.createTextureQuad(1.0, 1.0)) #barrera de contencion para usar
    Barreraa.texture = es.textureSimpleSetup(
        getAssetPath("wall2.jpg"), GL_REPEAT, GL_REPEAT, GL_LINEAR_MIPMAP_LINEAR, GL_NEAREST)
    glGenerateMipmap(GL_TEXTURE_2D)

    Barrera = sg.SceneGraphNode('plane') 
    Barrera.transform = tr.matmul([tr.scale(1,0.5,1),tr.translate(1.5,0,5),tr.rotationY(-np.pi/2)]) #se escala para que tenga la ubicacion que se necesita
    Barrera.childs += [Barreraa]
    scene = sg.SceneGraphNode('system')
    scene.childs += [Barrera]
    return scene
...
def createStaticScene(pipeline):
    ...
    Barreras=sg.SceneGraphNode('barreras') #se crean de forma iterativa los muros
    for i in range(10):
        barra= sg.SceneGraphNode('barrera'+str(i))
        barra.transform = tr.translate(0.0,0.0,-1.0*i)
        barra.childs += [createWall(pipeline)]
        Barreras.childs += [barra]
    for i in range(10):
        barra= sg.SceneGraphNode('barrera'+str(i))
        barra.transform = tr.translate(1.0,0.0,-1.0*i)
        barra.childs += [createWall(pipeline)]
        Barreras.childs += [barra]
    for i in range(10):
        barra= sg.SceneGraphNode('barrera'+str(i))
        barra.transform = tr.translate(-4.0,0.0,-1.0*i)
        barra.childs += [createWall(pipeline)]
        Barreras.childs += [barra]
    for i in range(10):
        barra= sg.SceneGraphNode('barrera'+str(i))
        barra.transform = tr.translate(-3.0,0.0,-1.0*i)
        barra.childs += [createWall(pipeline)]
        Barreras.childs += [barra]
    condo=sg.SceneGraphNode('condomio de casas') #se crea el conjunto de las casas
    for i in range(5):
        casas = sg.SceneGraphNode('casa'+str(i))
        casas.transform = tr.translate(5.0,0.0,-1.0*i)
        casas.childs += [createHouse(pipeline)]
        condo.childs += [casas]
    for i in range(5):
        casas = sg.SceneGraphNode('casa'+str(i))
        casas.transform = tr.translate(5.0,0.0,1.0*i)
    ...
    scene.childs += [condo] #se anexa todo a la escena final 
    scene.childs += [Barreras]
...
if __name__ == "__main__":
    ...
    t0 = glfw.get_time() #tiempo para el mov del auto 

    while not glfw.window_should_close(window):

        t1 = glfw.get_time() 
        dt = t1 - t0 #valor pequeño para el mov
        t0 = t1

        autoMov = sg.findNode(car, "car1") #nodo del mov del auto 

        if (glfw.get_key(window, glfw.KEY_A) == glfw.PRESS): 
            theta -= 2 * dt  #se modifica el angulo de a poco
            autoMov.transform = tr.matmul([autoMov.transform, tr.rotationY(2*dt)]) #mov hacia la izq 

        if (glfw.get_key(window, glfw.KEY_D) == glfw.PRESS):
            theta += 2* dt
            autoMov.transform = tr.matmul([autoMov.transform, tr.rotationY(-2*dt)]) #mov hacia la der 
        ...
        controller.camUp = np.array([0,1,0]) #no hay nesidad de modificar up

        # Actualizar la matriz
        view = tr.lookAt( #definimos view
            controller.viewPos,
            controller.at,
            controller.camUp
        )
